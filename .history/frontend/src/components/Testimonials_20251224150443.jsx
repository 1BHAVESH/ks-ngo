        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {testimonials.map((testimonial, index) => (
              <Card
                key={testimonial.id}
                className="border-sage p-6 transition-shadow hover:shadow-lg"
              >
                {/* Rating */}
                <div className="mb-4 flex gap-1">
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <Star
                      key={i}
                      className="h-5 w-5 fill-forest text-forest"
                    />
                  ))}
                </div>

                {/* Content */}
                <p className="mb-4 italic leading-relaxed text-earth">
                  “{testimonial.content}”
                </p>
