              ...
              <Card key={index} className="p-8 border-[#a3b49b] bg-[#fbfdf5] hover:shadow-lg transition-shadow">
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="flex-shrink-0">
                    <div className="w-20 h-20 rounded-full bg-[#dee9d4] flex items-center justify-center text-4xl">
                      {service.icon}
                    </div>
                  </div>
                  <div className="flex-1">
                    <h2 className="text-2xl font-bold text-[#0d3811] mb-3">{service.title}</h2>
                    <p className="text-[#65504a] leading-relaxed text-lg mb-4">{service.description}</p>
                    <ul className="space-y-2">
                      {service.details.map((detail, idx) => (
                        <li key={idx} className="flex items-start gap-2 text-[#65504a]">
